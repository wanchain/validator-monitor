...
exports.getDiskInfo = async function(diskName) {
  ...
  if (disk) {
    ...
    let usage, free;
    if (disk['Use%']) {
      usage = disk['Use%'].replace('%', '');
    } else if (disk.Available) {
      usage = (100 * (total - disk.Available) / total).toFixed(1);
    } else {
      usage = (100 * disk.Used / total).toFixed(1);
    }
    if (disk.Available) {
      free = disk.Available;
    } else {
      free = total - disk.Used;
    }
    let totalGb = (total / 1024).toFixed(1);
    let freeGb = (free / 1024).toFixed(1);
    return {usage, total: totalGb, free: freeGb};
  } else {
    return null;
  }
}